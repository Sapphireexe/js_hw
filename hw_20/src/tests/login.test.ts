...
import { VALID_REGISTRATION_DATA, INVALID_REGISTRATION_DATA, VALID_LOGIN_DATA, INVALID_LOGIN_DATA } from "../data/testdata";

describe("[UI] Registering smoke", () => {
  ...
  context('Negative scenarios', async function () {
    ...
    INVALID_REGISTRATION_DATA.forEach(data => {
      if(data.testName !== "Should not register with login that has already taken") {
  ...
  beforeEach(async function () {
    ...
    VALID_LOGIN_DATA.forEach(async data => {
      await browser.execute((data) => {
        localStorage.setItem(`${data.login}`, JSON.stringify({ name: data.login, password: data.password }));
      }, data);
    });
  });
  ...
  context('Positive scenarios', async function () {
    ...
    VALID_LOGIN_DATA.forEach(data => {
      ...
      it(`${data.testName}`, async function () {
        ...
        const actualMessage = await $(SELECTORS.successfulLoginMessage).getText();
